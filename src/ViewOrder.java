...
public class ViewOrder extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("orderId") == null) {
			pw.print(
					"<table align='center' class='table table-striped'><tr><td>Enter OrderNo &nbsp&nbsp</td><td><input name='orderId' type='text' class='form-control'></td></tr>");
			pw.print(
					"<tr><td colspan='2'>&nbsp&nbsp<input type='submit' name='Order' value='ViewOrder' class='btn btn-primary'></td></tr></table>");
		}
		...
		try {
			...
			// "/webapps/vinit/PaymentDetails.txt"));
			// ObjectInputStream objectInputStream = new ObjectInputStream(fileInputStream);

			// orderPayments = MySQLDataStoreUtilities.selectCustomerOrder(username,
			// orderid,"View");
		...
		if (request.getParameter("Order") != null && request.getParameter("Order").equals("ViewOrder")) {
			if (request.getParameter("orderId") != null && request.getParameter("orderId") != "") {
				...
				try {
					...
					// "/webapps/vinit/PaymentDetails.txt"));
					// ObjectInputStream objectInputStream = new ObjectInputStream(fileInputStream);
					orderPayments = MySQLDataStoreUtilities.selectCustomerOrder(username, orderid, "View");
				} catch (Exception e) {
